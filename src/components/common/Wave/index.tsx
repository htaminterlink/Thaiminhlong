import React from "react";
import styled from "styled-components";

interface WaveProps {
  colors?: string[];
  position?: "top" | "bottom";
  className?: string;
}

const WaveWrapper = styled.div<{ $position: string }>`
  pointer-events: none;
  position: absolute;
  ${({ $position }) => ($position === "top" ? "top: 0;" : "bottom: -1px;")}
  left: 0;
  width: 100%;
  z-index: 10;

  ${({ $position }) =>
    $position === "top" &&
    `
    transform: rotate(180deg);
  `}
`;

const WaveSvg = styled.svg`
  position: relative;
  width: 100%;
  height: 20rem;
  display: block;

  @media (max-width: 768px) {
    height: 15rem;
  }
`;

const ParallaxGroup = styled.g`
  use {
    animation: move-forever 25s cubic-bezier(0.55, 0.5, 0.45, 0.5) infinite;
    will-change: transform;
  }

  use:nth-child(1) {
    animation-delay: -2s;
    animation-duration: 7s;
  }

  use:nth-child(2) {
    animation-delay: -3s;
    animation-duration: 10s;
  }

  use:nth-child(3) {
    animation-delay: -4s;
    animation-duration: 13s;
  }

  use:nth-child(4) {
    animation-delay: -5s;
    animation-duration: 20s;
  }

  @keyframes move-forever {
    0% {
      transform: translate3d(-90px, 0, 0);
    }
    100% {
      transform: translate3d(85px, 0, 0);
    }
  }
`;

export const Wave: React.FC<WaveProps> = ({
  colors = [
    "rgba(67, 122, 242, 0.1)",
    "rgba(67, 122, 242, 0.2)",
    "rgba(67, 122, 242, 0.5)",
    "#4379F2",
  ],
  position = "bottom",
  className = "",
}) => {
  return (
    <WaveWrapper $position={position} className={`Wave ${className}`}>
      <WaveSvg
        className="waves"
        shapeRendering="auto"
        preserveAspectRatio="none"
        viewBox="0 24 150 28"
        xmlns="http://www.w3.org/2000/svg"
        xmlnsXlink="http://www.w3.org/1999/xlink"
      >
        <defs>
          <path
            id="gentle-wave"
            d="M-160 44c30 0 58-18 88-18s 58 18 88 18 58-18 88-18 58 18 88 18 v44h-352z"
          />
        </defs>
        <ParallaxGroup className="parallax">
          <use fill={colors[0]} y="0" x="48" xlinkHref="#gentle-wave" />
          <use fill={colors[1]} y="3" x="48" xlinkHref="#gentle-wave" />
          <use fill={colors[2]} y="5" x="48" xlinkHref="#gentle-wave" />
          <use fill={colors[3]} y="7" x="48" xlinkHref="#gentle-wave" />
        </ParallaxGroup>
      </WaveSvg>
    </WaveWrapper>
  );
};
